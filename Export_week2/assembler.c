// assembler.c  -- two-pass assembler for tiny ISA (LOAD, ADD, STORE, HALT)
// Usage: ./assembler input.asm output_base
...
int main(int argc, char **argv){
    ...
    for(int i=0;i<nlines;i++){
        ...
        if(strcmp(lower,"halt")==0){
            pc += 1;
        } else if(strcmp(lower,"load")==0 || strcmp(lower,"add")==0 || strcmp(lower,"store")==0){
            pc += 2;
        } else {
            char msg[128]; snprintf(msg,sizeof(msg),"Unknown mnemonic (pass1): %s (line %d)", toks[0], i+1); die(msg);
        }
        free_toks(toks, nt);
    }
    ...
    for(int i=0;i<nlines;i++){
                ...
                for(int k=1;k<nt;k++){
                    ...
                    while(tok2){
                        ...
                        if(!ok){
                            ...
                            if(idx<0){ char msg[128]; snprintf(msg,sizeof(msg),"Unknown symbol in .byte: %s (line %d)", tok2, i+1); die(msg); }
                            v = symtab[idx].value;
                        }
        ...
        if(strcmp(lower,"halt")==0){
            ...
            out_mem[pc] = OP_HALT; used[pc]=1; b0 = out_mem[pc]; nbytes_emitted=1; pc++;
        } else if(strcmp(lower,"load")==0 || strcmp(lower,"add")==0 || strcmp(lower,"store")==0){
            if(nt<2) { char m[128]; snprintf(m,sizeof(m),"Missing operand for %s (line %d)", toks[0], i+1); die(m); }
            int op = (strcmp(lower,"load")==0)? OP_LOAD : (strcmp(lower,"add")==0? OP_ADD : OP_STORE);
            ...
            if(!ok){ char m[128]; snprintf(m,sizeof(m),"Undefined operand: %s (line %d)", toks[1], i+1); die(m); }
            out_mem[pc] = (uint8_t)(val & 0xFF); used[pc]=1; b1 = out_mem[pc]; pc++;
            nbytes_emitted = 2;
        } else {
            char msg[128]; snprintf(msg,sizeof(msg),"Unknown mnemonic (pass2): %s (line %d)", toks[0], i+1); die(msg);
        }
